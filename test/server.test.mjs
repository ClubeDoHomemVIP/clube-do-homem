...
test('página pública de oferta está disponível',async()=>{const r=await fetch(`http://localhost:${port}/oferta`);assert.equal(r.status,200);const html=await r.text();assert.match(html,/R\$<\/sup><b>19/);assert.match(html,/Vitalício/);assert.match(html,/CONHEÇA O CLUBE/);assert.match(html,/https:\/\/t\.me\/previasclubedohomem/);assert.match(html,/cobrança aparece de forma discreta/)});
